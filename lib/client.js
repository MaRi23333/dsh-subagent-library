...
function LibrarySettings(props) {
	...
	*  other rows. External pushes (other windows/tools) still reload. */
	const skipNextPush = (0, react.useRef)(false);
	(0, react.useEffect)(() => () => {
		alive.current = false;
	}, []);
	const load = () => {
	...
	const applyWrite = async (write) => {
		...
		try {
			...
			if (result.ok) {
				...
				setEntries((current) => {
					const next = { ...current };
					if (write.op === "save") for (const [id, entry] of Object.entries(write.entries)) next[id] = structuredClone(entry);
					else delete next[write.id];
					return next;
				});
				setStatus({
					kind: "ok",
...
async function readView() {
	...
	if (!response.ok || typeof body !== "object" || body === null || body.ok !== true) {
		...
		throw new Error((error !== void 0 ? ERROR_TEXT[error] : void 0) ?? "子代理库接口不可用（插件未加载？）");
	}
	const value = body;
	return {
